...
//       const response2 = await publicAPI("getcontact", "GET")
...
export function Contact() {
  ...
  /* ---------- Coordonnées auteur ---------- */
  const [contactInfo, setContactInfo] = useState<AuthorContact>({
    email_contact: '',
    telephone: ''
  });
  ...
  useEffect(() => {
    const fetchContactInfo = async () => {
      try {
        ...
        console.error('Erreur récupération contact auteur', err);
      } finally {
  ...
  const handleChange = (
    ...
    // Capitaliser automatiquement certains champs
    if (name === 'nom' || name === 'sujet') {
      processedValue = capitalizeFirst(value);
    } else if (name === 'message') {
      processedValue = capitalizeSentences(value);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50">
      {/* En-tête responsive */}
      <div className="bg-gradient-to-r from-amber-100 to-orange-100 py-4 sm:py-6 md:py-8 border-b-2 border-amber-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-xs sm:text-sm text-amber-700 font-serif mb-1">Échanges littéraires</p>
              <h1 className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-serif font-bold text-amber-900 leading-tight">
                Correspondance
              </h1>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 md:py-16 grid grid-cols-1 lg:grid-cols-5 gap-6 sm:gap-8 md:gap-12">
        ...
        <div className="lg:col-span-2 space-y-4 sm:space-y-6">
          ...
          <div className="bg-gradient-to-br from-white to-amber-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 shadow-lg border-2 border-amber-200">
            <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 md:mb-6">
              <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-gradient-to-br from-amber-200 to-orange-200 rounded-full flex items-center justify-center">
                <MessageSquare className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-amber-800" />
              </div>
              <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-serif font-bold text-amber-900">
                Écrivons ensemble
              </h2>
            </div>
            <p className="text-sm sm:text-base md:text-lg text-amber-800 font-serif leading-relaxed mb-3 sm:mb-4 md:mb-6">
              J'ai toujours cru que la littérature est un dialogue entre l'auteur et ses lecteurs. 
              Vos mots, vos réflexions et vos questions nourrissent ma créativité.
            </p>
            <p className="text-xs sm:text-sm md:text-base text-amber-700 font-serif leading-relaxed">
              N'hésitez pas à me faire part de vos impressions sur mes œuvres ou à partager votre passion pour les mots.
            </p>
          </div>

          <div className="bg-gradient-to-br from-orange-50 to-yellow-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 shadow-lg border-2 border-orange-200">
            <h3 className="text-lg sm:text-xl md:text-2xl font-serif font-bold text-amber-900 mb-4 sm:mb-6">
              Mes coordonnées
            </h3>

            {loadingContact ? (
              <p className="text-sm sm:text-base text-amber-700 font-serif">Chargement…</p>
            ) : (
              <div className="space-y-4 sm:space-y-6">
                {/* Email */}
                <div className="flex items-start gap-3 sm:gap-4">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm text-amber-600 font-serif mb-1">Email</p>
                    <a
                      href={`mailto:${contactInfo.email_contact}`}
                      className="text-sm sm:text-base font-serif font-semibold text-amber-900 hover:text-amber-700 transition-colors break-all"
                    >
                      {contactInfo.email_contact || 'Non renseigné'}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 sm:gap-4">
                  ...
                  <div>
                    ...
                    <a
                      ...
                      className="text-sm sm:text-base font-serif font-semibold text-amber-900 hover:text-amber-700 transition-colors"
                    >
                      {contactInfo.telephone || 'Non renseigné'}
                    </a>
                  </div>
                </div>
          ...
          {/* Horaires de réponse */}
          <div className="bg-gradient-to-br from-amber-100 to-orange-100 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 shadow-lg border-2 border-amber-300">
            <h3 className="text-base sm:text-lg md:text-xl font-serif font-bold text-amber-900 mb-3 sm:mb-4">
              Temps de réponse
            </h3>
            <p className="text-xs sm:text-sm md:text-base text-amber-800 font-serif leading-relaxed">
              Je m'efforce de répondre à tous les messages dans un délai de 48 à 72 heures. 
        ...
        {/* =======================
            FORMULAIRE
        ======================= */}
        <div className="lg:col-span-3 bg-gradient-to-br from-white to-amber-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 lg:p-10 shadow-lg border-2 border-amber-200">
          <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 md:mb-8">
            <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-gradient-to-br from-amber-200 to-orange-200 rounded-full flex items-center justify-center">
              <Send className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-amber-800" />
            </div>
            <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-serif font-bold text-amber-900">
              Envoyez-moi un message
            </h2>
          ...
          {submitStatus === 'error' && (
            <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-red-50 border-2 border-red-200 rounded-lg sm:rounded-xl flex items-center gap-2 sm:gap-3">
              <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-red-600 flex-shrink-0" />
              <div className="text-red-800 font-serif text-sm sm:text-base">
                <p className="font-semibold mb-1">Erreur lors de l'envoi</p>
                <p>{errorMessage || 'Une erreur s\'est produite. Veuillez réessayer.'}</p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            >
              ...
              {/* Messages de statut */}
          {submitStatus === 'success' && (
            <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-green-50 border-2 border-green-200 rounded-lg sm:rounded-xl flex items-center gap-2 sm:gap-3">
