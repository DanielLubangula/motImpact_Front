    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50">
      <div className="max-w-6xl mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-5 gap-12">
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-4">
                  ...
                  <a
                    ...
                    className="font-serif font-semibold text-amber-900 hover:text-amber-700"
                  >
        ...
        {/* =======================
            FORMULAIRE
        ======================= */}
        <div className="lg:col-span-3 bg-white p-10 rounded-2xl border-2 border-amber-200 shadow-lg">
          <h2 className="text-3xl font-serif font-bold text-amber-900 mb-6">
            Envoyez-moi un message
          </h2>

          {submitStatus === 'success' && (
            <div className="mb-4 flex gap-3 text-green-700">
              <CheckCircle /> Message envoyé avec succès
            </div>
          )}

          {submitStatus === 'error' && (
            <div className="mb-4 flex gap-3 text-red-700">
              <AlertCircle /> {errorMessage}
            </div>
          )}

          <form onSubmit={handleSubmit} className="space-y-5">
            <input name="nom" value={formData.nom} onChange={handleChange} placeholder="Nom" className="w-full p-3 border rounded-xl" />
            <input name="email" value={formData.email} onChange={handleChange} placeholder="Email" className="w-full p-3 border rounded-xl" />
            <select name="sujet" value={formData.sujet} onChange={handleChange} className="w-full p-3 border rounded-xl">
              <option value="">Sujet</option>
              <option value="question">Question</option>
              <option value="collaboration">Collaboration</option>
            </select>
            <textarea name="message" value={formData.message} onChange={handleChange} rows={5} className="w-full p-3 border rounded-xl" />
            <button disabled={isSubmitting} className="w-full bg-amber-800 text-white p-4 rounded-xl">
