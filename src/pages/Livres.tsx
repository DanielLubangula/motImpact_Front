...
import { useEffect, useState } from 'react';
import { Link } from 'react-router-dom';
import { BookOpen, Download, DollarSign, Gift, Search, Filter, Star, Coins, Loader2 } from 'lucide-react';
import { publicAPI } from '../lib/api';
import { Livre } from '../types';
...
export function Livres() {
  ...
  const LoadingBookCard = () => (
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-md border border-amber-200 overflow-hidden animate-pulse">
      <div className="relative aspect-[4/3] sm:aspect-[3/4] bg-gradient-to-br from-amber-100 to-orange-100">
        <div className="absolute inset-0 flex items-center justify-center">
          <Loader2 className="w-8 h-8 sm:w-10 sm:h-10 text-amber-400 animate-spin" />
        </div>
      </div>
      <div className="p-3 sm:p-4 md:p-5">
        <div className="h-4 sm:h-5 bg-amber-200 rounded mb-2 sm:mb-3"></div>
        <div className="space-y-1 sm:space-y-2">
          <div className="h-3 sm:h-4 bg-amber-100 rounded"></div>
          <div className="h-3 sm:h-4 bg-amber-100 rounded"></div>
        </div>
        <div className="flex gap-2 mt-3 sm:mt-4">
          <div className="flex-1 h-8 sm:h-10 bg-amber-200 rounded-lg"></div>
          <div className="flex-1 h-8 sm:h-10 bg-amber-200 rounded-lg"></div>
        </div>
      </div>
    </div>
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 lg:px-12 py-4 sm:py-6 md:py-8 lg:py-12">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3 md:gap-4 lg:gap-6 mb-4 sm:mb-6 md:mb-8 lg:mb-12">
          <div className="bg-gradient-to-br from-white to-green-50 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 lg:p-6 shadow-sm sm:shadow-md border border-green-200 text-center">
            ...
            <p className="text-sm sm:text-base md:text-lg lg:text-xl font-serif font-bold text-green-800">
              {loading ? (
                <span className="inline-block w-8 h-4 sm:h-5 md:h-6 bg-green-200 rounded animate-pulse"></span>
              ) : (
                livres.filter(l => l.statut === 'gratuit').length
              )}
            </p>
          ...
          <div className="bg-gradient-to-br from-white to-amber-50 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 lg:p-6 shadow-sm sm:shadow-md border border-amber-200 text-center">
            ...
            <p className="text-sm sm:text-base md:text-lg lg:text-xl font-serif font-bold text-amber-800">
              {loading ? (
                <span className="inline-block w-8 h-4 sm:h-5 md:h-6 bg-amber-200 rounded animate-pulse"></span>
              ) : (
                livres.filter(l => l.statut === 'payant').length
              )}
            </p>
          ...
          <div className="bg-gradient-to-br from-white to-orange-50 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 lg:p-6 shadow-sm sm:shadow-md border border-orange-200 text-center">
            ...
            <p className="text-sm sm:text-base md:text-lg lg:text-xl font-serif font-bold text-orange-800">
              {loading ? (
                <span className="inline-block w-8 h-4 sm:h-5 md:h-6 bg-orange-200 rounded animate-pulse"></span>
              ) : (
                livres.length
              )}
            </p>
        ...
        {/* NOUVEAU: Grille de chargement pendant le chargement */}
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
            {[...Array(6)].map((_, index) => (
